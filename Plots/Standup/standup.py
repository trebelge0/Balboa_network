...
ax.set_xlim(first_change_time - start_time - 10, last_end_time - start_time-40)
...
# Légende
ready_patch = mpatches.Patch(color="mediumseagreen", label="Balancing")
not_ready_patch = mpatches.Patch(color="lightcoral", label="Resting")
ax.legend(handles=[not_ready_patch, ready_patch], loc="upper right")

plt.tight_layout()
plt.savefig('standup_res.png')
plt.show()
