...
class Balancer:
  ...
  def stand_up(self):
    if self.calibrated:
      ...
      self.reset()
      self.imu.read()
      self.rocky.motors(-sign*200, -sign*200)
      time.sleep(0.3)
      self.rocky.motors(sign*200, sign*200)
  ...
  def update_loop(self):
    while self.running:
      ...
      fallingAngleOffset = self.angle_rate * ANGLE_RATE_RATIO / 1000 - (self.angle - 10)
      if fallingAngleOffset > 0:
        self.rocky.leds(0,1, 0)
      else:
        self.rocky.leds(0, 0, 1)

    # stop() has been called and the loop has exited. Stop the motors.
    self.rocky.motors(0, 0)
  ...
  def integrate_encoders(self):
    (counts_left, counts_right) = self.rocky.read_encoders()
  ...
  def balance(self):
    ...
    rising_angle_offset = self.angle_rate * ANGLE_RATE_RATIO/1000 + (self.angle-10)
